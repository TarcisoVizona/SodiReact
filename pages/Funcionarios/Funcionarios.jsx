...
export function Funcionarios() {
  const { dados, setControle, controle, handleDeleteEmployee } = useFuncionariosHook();

  return (
    <div>
      <div id="principal" className="flex flex-wrap justify-center gap-10 p-6">
        {dados.map((element) => {
          return (
            <div
              key={element.id_usuario}
              className="w-65 h-fit shadow-2xl p-4 rounded-2xl border border-gray-100 flex flex-col gap-2"
            >
              ...
              <div className="flex justify-around mt-5 mb-5">
                <button
                  onClick={() => {
                    handleDeleteEmployee(element.id_usuario);
                  }}
